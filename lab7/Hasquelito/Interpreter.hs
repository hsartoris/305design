...
interpreter (Let s ast1 ast2) env 	= interpreter ast2 ((s, ast1):env)
interpreter (Lambda x ast1 type1 type2) env = (Lambda x (subst(remove_var x env) ast1) type1 type2)
interpreter (App ast1 ast2) env 	= let (Lambda x e type1 type2) = interpreter ast1 env in interpreter e ((x, ast2):env)
